...
- Added lusca to the project 
    - to protect against XSS attacks. Lusca is a middleware that provides security headers for Express applications. It is used to set the X-XSS-Protection header to 1; mode=block, which tells the browser to block any detected XSS attacks. It also sets the X-Content-Type-Options header to nosniff, which prevents the browser from interpreting files as a different MIME type than what is specified in the Content-Type header. This helps prevent certain types of attacks, such as drive-by downloads and content injection.

    - Also added CSRF protection to the project. Lusca provides CSRF protection by generating a unique token for each user session and validating it on subsequent requests. This helps prevent attackers from making unauthorized requests on behalf of the user. The CSRF token is included in the response as a hidden input field in forms, and it must be included in the request body when submitting the form. If the token is missing or invalid, the request will be rejected with a 403 Forbidden status code.

- Added express-rate-limit to the project to limit the number of requests that can be made to the server in a given time period. This helps prevent brute-force attacks and denial-of-service attacks. The rate limiter is set to allow a maximum of 5 requests per minute from a single IP address. If the limit is exceeded, the server will respond with a 429 Too Many Requests status code and an error message.
